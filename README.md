...
Делает краткое саммари последних 100 сообщений. Учитывает имена участников, не цензурирует мат из переписки и отвечает неформально.
...
Для `/summary` лимит персональный для обычных участников:

```text
1 запрос за последние 24 часа
```
...
```text
Вы кого задудосить решили дети шлюх ебаных
```

Для `/drama`, `/top_words`, `/who`, `/lore` лимит персональный:

```text
каждый обычный участник может вызвать каждую команду 1 раз в 24 часа
```
